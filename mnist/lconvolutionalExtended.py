...
#ITERATIONS = int(num_epochs * train_size) // BATCH_SIZE GOOGLE Original
ITERATIONS = 10000
...
def loadFromKaggle(filename,extractLabels,nskip=1):
  data = numpy.loadtxt(filename,skiprows=nskip,ndmin=2,delimiter=',')
  if extractLabels == True:
    labels = data[:,0]
    data = data[:,1:]
    data = (data - (PIXEL_DEPTH / 2.0)) / PIXEL_DEPTH
    data = data.reshape(data.shape[0], IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNELS)
    return data,labels
  else:
    data = (data - (PIXEL_DEPTH / 2.0)) / PIXEL_DEPTH
    data = data.reshape(data.shape[0], IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNELS)
...
def main(_):
    train_data,train_labels = loadFromKaggle('train.csv', True,1)
    test_data = loadFromKaggle('test.csv', False,1)
...
#Laura was here....
    conv3_weights = tf.Variable(tf.truncated_normal(
        ...
        seed=SEED, dtype=data_type()), name = 'conv3_weights')
    conv3_biases = tf.Variable(tf.constant(0.1, shape=[128], dtype=data_type()),name = 'conv3_biases')
    
    fc1_weights = tf.Variable(  # fully connected, depth 512.
        #tf.truncated_normal([IMAGE_SIZE // 4 * IMAGE_SIZE // 4 * 64, 512], Original
    ...
    #Save model to do a ensemble later
